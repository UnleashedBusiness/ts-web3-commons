...
export class Erc20TokenContract extends BaseMultiChainContract<typeof Erc20Abi> {
    ...
    //CUSTOM
    public async decimals(config: BlockchainDefinition, address: string, batch?: Web3BatchRequest, callback?: (result: number) => void) {
        if (!this.decimalsCache.has(config.networkId))
            this.decimalsCache.set(config.networkId, new Map<string, number>());
        if (!this.decimalsCache.get(config.networkId)?.has(address))
            this.decimalsCache.get(config.networkId)?.set(address, await this.getPropertyMulti(config, address, "decimals") as number)
        if (callback) {
    ...
    public async symbol(config: BlockchainDefinition, address: string, batch?: any, callback?: (result: string) => void) {
        return this.getPropertyMulti(config, address, "symbol", batch, callback);
    }

    public async name(config: BlockchainDefinition, address: string, batch?: any, callback?: (result: string) => void) {
        return this.getPropertyMulti(config, address, "name", batch, callback);
    }
    ...
    public async balanceOf(config: BlockchainDefinition, contractAddr: string, address: string, batch?: any, callback?: (balance: BigNumber) => void) {
        if (typeof callback !== "undefined") {
            return this.getViewMulti(
                ...
                batch,
                async (result: number) => callback ? callback(this.wrap(result).dividedBy(10 ** this.decimalsSync(config, contractAddr))) : undefined);
        } else {
            const amount = await this.getViewMulti(
                config,
                contractAddr,
                // @ts-ignore
                async (contract) => contract.methods.balanceOf(address)) as number;
            return this.wrap(amount).dividedBy(10 ** this.decimalsSync(config, contractAddr))
        }
    }

    public totalSupply(config: BlockchainDefinition, contractAddr: string, batch?: any, callback?: (supply: BigNumber) => void) {
        return this.getViewMulti(
            config,
            contractAddr,
            // @ts-ignore
    ...
    public async allowanceForSpender(config: BlockchainDefinition, wallet: string, contractAddr: string, spender: string, batch?: any, callback?: (supply: BigNumber) => void) {
        if (typeof callback !== "undefined") {
            return this.getViewMulti(
                config,
                contractAddr,
                ...
                batch,
                (result: number) => callback ? callback(this.wrap(result).dividedBy(10 ** this.decimalsSync(config, contractAddr))) : undefined);
        } else {
            const amount = await this.getViewMulti(
                config,
                contractAddr,
                // @ts-ignore
                async (contract) => contract.methods.allowance(wallet, spender)) as number;
            return this.wrap(amount).dividedBy(10 ** this.decimalsSync(config, contractAddr))
        }
    }
    ...
    async transfer(contractAddr: string, to: string, amount: BigNumber): Promise<void> {
        ...
        const amountWei = amount.multipliedBy(division);
        // @ts-ignore
        await this.runMethodConnectedMulti(contractAddr, async (contract, _) => contract.methods.transfer(to, amountWei.toString()))
    }
    ...
    async transferRunnable(contractAddr: string, to: string, amount: BigNumber) {
        ...
        const amountWei = amount.multipliedBy(division);
        // @ts-ignore
        return this.buildMethodRunnableMulti(contractAddr, async (contract, _) => contract.methods.transfer(to, amountWei.toString()))
    }
    ...
    protected override async initMultiChainContractReadonly(config: BlockchainDefinition, address: string): Promise<void> {
