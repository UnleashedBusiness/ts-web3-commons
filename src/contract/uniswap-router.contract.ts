...
export class UniswapRouterContract extends BaseTokenAwareContract {
    ...
    public async getAmountsOut(
        ...
        const amountInBN = new BigNumber(amountIn).multipliedBy(amountInDivision).decimalPlaces(0);

        const division = await this.tokenDivision(config, path[path.length - 1]);
        const localResult = await this.getViewMulti<number[]>(
            config,
            routerAddr,
            (contract) => contract.methods.getAmountsOut(amountInBN.toString(), path), batch,
            callback !== undefined
                ? (result: number[]) => {
                    if (callback) callback(this.wrap(result[path.length - 1]).dividedBy(division))
                }
                : undefined
        ) as number[];
        if (callback)
            ...
            return this.wrap(localResult[path.length - 1] as number).dividedBy(division);
    }
    ...
    public async getAmountsIn(
        ...
        const amountOutDivision = await this.tokenDivision(config, path[path.length - 1]);
        const amountOutBN = new BigNumber(amountOut).multipliedBy(amountOutDivision).decimalPlaces(0);
        ...
        if (callback)
            ...
            return this.wrap(localResult[0] as number).dividedBy(division);
    }
    ...
    public factory(
        ...
        return this.getPropertyMulti(config, routerAddr, 'factory', batch, callback);
    }
    ...
    public async swapExactETHForTokens(
        ...
        const amountOutDivision = await this.tokenDivision(this.walletConnection.blockchain, path[path.length - 1]);
        const amountOutBN = new BigNumber(minAmountOut).multipliedBy(amountOutDivision).decimalPlaces(0);

        return this.runMethodConnectedMulti(routerAddr,
            (contract, connectedAddress) => contract.methods.swapExactETHForTokensSupportingFeeOnTransferTokens(
    ...
    public async swapExactTokensForTokens(
        ...
        const amountInDivision = await this.tokenDivision(this.walletConnection.blockchain, path[0]);
        const amountInBN = new BigNumber(amountIn).multipliedBy(amountInDivision).decimalPlaces(0);
        const amountOutDivision = await this.tokenDivision(this.walletConnection.blockchain, path[path.length - 1]);
        const amountOutBN = new BigNumber(minAmountOut).multipliedBy(amountOutDivision).decimalPlaces(0);
        ...
        return this.runMethodConnectedMulti(
            routerAddr,
            (contract, connectedAddress) => contract.methods.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                amountInBN.toString(),
                amountOutBN.toString(),
                path,
                connectedAddress,
    ...
    public async addLiquidityETH(
        ...
        const amountInDivision = await this.tokenDivision(this.walletConnection.blockchain, tokenAddress);
        ...
        return this.buildMethodRunnableMulti(routerAddr,
            (contract, connectedAddress) => contract.methods.addLiquidityETH(
                tokenAddress,
                amountIn.toString(),
    ...
    public async removeLiquidityETH(
        ...
        const liquidityDivision = await this.tokenDivision(this.walletConnection.blockchain, liquidityToken);
        const amountInDivision = await this.tokenDivision(this.walletConnection.blockchain, tokenAddress);
        liquidityAmount = liquidityAmount.multipliedBy(liquidityDivision).decimalPlaces(0);
        amountInMin = amountInMin.multipliedBy(amountInDivision).decimalPlaces(0);
        amountInWETHMin = amountInWETHMin.multipliedBy(10 ** 18).decimalPlaces(0);

        return this.buildMethodRunnableMulti(routerAddr,
            (contract, connectedAddress) => contract.methods.removeLiquidityETH(
                tokenAddress,
    ...
    public async swapExactTokensForETH(
        ...
        const amountInDivision = await this.tokenDivision(this.walletConnection.blockchain, path[path.length - 1]);
        const amountInBN = new BigNumber(amountIn).multipliedBy(amountInDivision).decimalPlaces(0);
        const amountOutBN = new BigNumber(minAmountOut).multipliedBy(10 ** 18).decimalPlaces(0);

        return this.runMethodConnectedMulti(routerAddr,
            (contract, connectedAddress) => contract.methods.swapExactTokensForETHSupportingFeeOnTransferTokens(
                amountInBN.toString(),
                amountOutBN.toString(),
                path,
                connectedAddress,
