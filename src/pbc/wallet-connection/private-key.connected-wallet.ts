...
export class PrivateKeyConnectedWallet implements ConnectedWalletInterface {
    ...
    public async signAndSendTransaction(payload: TransactionPayload<Rpc>, cost: string | number | undefined = 0): Promise<ShardPutTransactionResponse> {
        ...
        if (accountData.code !== 200 || !accountData.data) {
            throw new Error(`Account data request returned bad response! Code: ${accountData.code}, Data: ${accountData.data}`);
        }
        ...
        if (txPointer.code !== 200 || !txPointer.data) {
            throw new Error(`Transaction put request returned bad response! Code: ${accountData.code}, Data: ${txPointer.data}`);
        }

        return {
            putSuccessful: true,
            shard: txPointer.data.destinationShardId,
            transactionHash: txPointer.data.identifier,
        };
    }
}